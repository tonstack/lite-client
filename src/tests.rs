...
fn test_ecdh() {
    ...
    let their_public = PublicKey::from(to_montgomery(&their_public));
    ...
    let expected_public = to_montgomery(&expected_public).to_bytes();
    let ecdh = our_private.diffie_hellman(&their_public).to_bytes();
    let our_public = PublicKey::from(&our_private);
    assert_eq!(our_public.to_bytes(), expected_public, "pub");
    assert_eq!(x25519_dalek::x25519(our_private.to_bytes(), their_public.to_bytes()), expected_ecdh, "ecdh1");
    assert_eq!(ecdh, expected_ecdh, "ecdh2");
}