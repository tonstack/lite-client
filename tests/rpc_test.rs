use hex::FromHex;
use liteclient::{LiteClient, tl_types::*};
...
fn get_state_test() {
    ...
    let seqno: u32 = 2;
    let root_hash = liteclient::tl_types::Int256::from_base64("4bzgnFItQjTVEMYL9c/VHshMJttG9gDIXCzsMQdjKSU=").unwrap();
    let file_hash = liteclient::tl_types::Int256::from_base64("2gOSTo8fuMWgA18snVD1RUtTfpU5LvCQWOOQ16Z7w5Y=").unwrap();
    ...
    if result.is_err() {
        assert!(result.clone().unwrap_err().message == liteclient::tl_types::String::new("state not in db".to_string()) || result.unwrap_err().message == liteclient::tl_types::String::new("state already gc'd".to_string()));
    }
    else {
...
fn get_account_state_test() {
    ...
    let res = client.get_account_state(BlockIdExt{workchain, shard, seqno, root_hash: root_hash.clone(), file_hash: file_hash.clone()}, account).unwrap();
...
fn get_all_shards_info_test() {
    ...
    let mut client = LiteClient::connect(&config).unwrap();
    let block = (client).lookup_block(BlockId { workchain: -1, shard: 9223372036854775808, seqno: 28330758}, None, None).unwrap();
...
fn get_one_transaction_test() {
    ...
    let account = AccountId::from_friendly("EQAns9gBUsFtRnoOlwUuXW0738ol8CO5l46PlftBpzhRxmCs").unwrap();
...
fn get_transactions_test() {
    ...
    // let id = BlockIdExt{workchain: 0, seqno: 34073780, shard: 9223372036854775808, root_hash: Int256::from_base64("6JhaCv30PM3S4H5Zve/OMPHXUWv/LY0hOd9+M41VUi4=").unwrap(), file_hash: Int256::from_base64("2dL2E0uqZ5uDlKcPM11GeIPyaQgA4Hy7Zv3j2bKBhoI=").unwrap()};
    let account = AccountId::from_friendly("EQAns9gBUsFtRnoOlwUuXW0738ol8CO5l46PlftBpzhRxmCs").unwrap();
    let result =  client.get_transactions(10, account, 36479378000003, Int256::from_base64("2JoweA1e8Lz4hDO1KZd1tyTYXsIXWv3FbkWeRL+PvsM=").unwrap());
    assert!(result.is_ok());
}
...
fn lookup_block_test() {
    ...
    let mut client = LiteClient::connect(&config).unwrap();
    let block = liteclient::tl_types::BlockId{workchain: -1, shard: 3, seqno: 28327268};
    let utime = 1679764594;
    let lt = 36333154000001;
    let empty_block = liteclient::tl_types::BlockId{workchain: -1, shard: 1, seqno: 1};
    let res1 = client.lookup_block(block.clone(), None, None).unwrap();
    let res2 = client.lookup_block(empty_block.clone(), Some(lt), None).unwrap();
    let res3 = client.lookup_block(empty_block.clone(), None, Some(utime)).unwrap();
    assert_eq!(res1, res2);
    assert_eq!(res1, res3);
}
...
fn list_block_transactions_test() {
    ...
    let mut client = LiteClient::connect(&config).unwrap();
    let id = BlockIdExt{workchain: -1, seqno: 28455017, shard: 9223372036854775808, root_hash: Int256::from_base64("Re9R5ML5bOjRDb1H8/hDZIcVDZ6UuhzIlL38J9omfes=").unwrap(), file_hash: Int256::from_base64("kjP+8ReqSSiB6a6KKh9NKr/lov3RhFNZm0e/6Lndguc=").unwrap()};
...
fn get_block_proof_test() {
    ...
    let response: Vec<u8>;
    if let Message::Answer { query_id: _, answer} = result {
        response = answer;
    }
    else {panic!();}
...
fn get_validator_stats_test() {
    ...
    let mut client = LiteClient::connect(&config).unwrap();
    let id = BlockIdExt{workchain: -1, seqno: 28455017, shard: 9223372036854775808, root_hash: Int256::from_base64("Re9R5ML5bOjRDb1H8/hDZIcVDZ6UuhzIlL38J9omfes=").unwrap(), file_hash: Int256::from_base64("kjP+8ReqSSiB6a6KKh9NKr/lov3RhFNZm0e/6Lndguc=").unwrap()};
    let result = client.get_validator_stats(id.clone(), 10, Some(Int256::from_base64("Re9R5ML5bOjRDb1H8/hDZIcVDZ6UuhzIlL38J9omfes=").unwrap()), None);
    assert!(result.is_ok());
    let result = client.get_validator_stats(id.clone(), 10, Some(Int256::from_base64("Re9R5ML5bOjRDb1H8/hDZIcVDZ6UuhzIlL38J9omfes=").unwrap()), Some(1650613847));
    assert!(result.is_ok());
